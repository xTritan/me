...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        # Filter pets whose names contain the letter
        filtered_pets = pet_filter(letter)
        # Count the number of pets with the letter
...
def make_filler_text_dictionary() -> Dict:
    ...
    for word_length in range(3, 8):
        url = f"{url_base}{word_length}"
        word = [requests.get(url).text for _ in range(4)]
        wd[word_length] = word
    return wd
